...
export class Rotation implements Interface.Transformation {
    ...
    constructor(private container: Container, private selection: PIXI.Rectangle) {
        ...
        this.container.transformation.scale.x.on('change', () => this.updateRotator());
        this.container.transformation.scale.y.on('change', () => this.updateRotator());
        Pubsub.on('camera:update', camera => {
            this.zoom = camera.zoom;
            this.rotator.scale.set(Math.sign(this.container.scale.x) / this.container.scale.x / camera.zoom,
                                Math.sign(this.container.scale.y) / this.container.scale.y / camera.zoom);
    ...
    mousemove(position: PIXI.Point, clickedPosition: PIXI.Point): boolean {
        ...
        this.container.transformation.rotation.value = newAngle - this.lastRotation ;
        if (Selection.snapToAngle) {
            let snapAngle = Selection.angleSnap * PIXI.DEG_TO_RAD;
            if (snapAngle !== 0)
                this.container.transformation.rotation.value = Math.round(this.container.rotation / snapAngle) * snapAngle;
        }
        return true;
    }
    ...
    update(children): void {
        this.container.removeChild(this.rotator);
        this.lclBndsY = this.container.getLocalBounds().y;
        this.rotator.scale.set(1, 1);
        this.rotator.pivot.set(0,0);
        this.rotator.position.x = this.container.pivot.x;
        this.updateRotator();
        this.container.addChild(this.rotator);
    }
}
